...
import type { ChatMessage } from "../../../shared/types/app.types.js";
...
export function useAiSpeech() {
    ...
    const pollyAudioCacheRef = useRef<Map<string, SpeechResult>>(new Map());
    ...
    function clearCurrentAudio({ resetSource = true }: { resetSource?: boolean } = {}): void {
    ...
    async function playAudioUrl({ messageId, audioUrl, requestId }: PlayAudioOptions): Promise<void> {
        ...
        setSpeakingMessageId(String(messageId));
        await audio.play();
    }
    ...
    async function speakMessage(message: ChatMessage): Promise<void> {
        ...
        if (speakingMessageId === String(message.id)) {
            stopSpeech();
            return;
        }
