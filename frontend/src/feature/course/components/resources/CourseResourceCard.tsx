...
function CourseResourceCard({ currentUser, resource, handleResourceClick, handleEditResource, handleToggleResource, handleDeleteResource }) {
    ...
    function handleRightClick(event: MouseEvent<HTMLElement>): void {
        if (!isTeacher) return;
    ...
    useEffect(() => {
        function handleClickOutside(event: globalThis.MouseEvent): void {
            if (resourceRef.current && event.target instanceof Node && !resourceRef.current.contains(event.target)) {
                setShowMenu(false);
            }
        }
