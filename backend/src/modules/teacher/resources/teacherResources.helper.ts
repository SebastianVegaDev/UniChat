...
export async function deleteResourceFile(fileUrl: string | null | undefined): Promise<void> {
    await deleteUploadFileByUrl(fileUrl, UPLOAD_PUBLIC_PATHS.resources);
}

export function getResourceFileData(file?: Express.Multer.File): {
    kind: string;
    sizeBytes: number;
    fileUrl: string | null;
} | null {
...
function getResourceKind(file?: Express.Multer.File): string {
