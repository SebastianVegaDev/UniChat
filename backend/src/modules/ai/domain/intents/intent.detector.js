...
export function detectDirectIntent(question) {
...
export function getDateFilter(question) {
    const cleanQuestion = normalizeText(question);
...
export function getRequestedEventTypes(question) {
    const cleanQuestion = normalizeText(question);
    const types = new Set();
...
export function isNextClassQuestion(question) {
    return includesAny(normalizeText(question), NEXT_CLASS_KEYWORDS);
}

export function getRequestedWeeks(question) {
    const cleanQuestion = normalizeText(question);
    const weeks = new Set();
...
export function getSpecificTokens(question, analysis = {}) {
...
function getIntentKeywordTokens() {
    return new Set([
...
function getCourseTokens(courseFilter) {
