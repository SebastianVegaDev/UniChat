...
import type { AiCalendarEvent, AiClassSession, AiResource } from "../types/ai.types.js";

export async function findAiAccessibleResources(userId: EntityId): Promise<AiResource[]> {
    const { rows } = await pool.query<AiResource>(`
        WITH auth_user AS (
            SELECT role
            FROM users
            WHERE id = $1
...
export async function findAiAccessibleCalendarEvents(userId: EntityId): Promise<AiCalendarEvent[]> {
    const { rows } = await pool.query<AiCalendarEvent>(`
        WITH auth_user AS (
            SELECT role
            FROM users
            WHERE id = $1
...
export async function findAiAccessibleClassSessions(userId: EntityId): Promise<AiClassSession[]> {
    const { rows } = await pool.query<AiClassSession>(`
        WITH auth_user AS (
            SELECT role
            FROM users
            WHERE id = $1
